    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-64 bg-slate-900 text-white flex flex-col h-full shrink-0">
            ...
            <div class="p-4 bg-slate-950 border-t border-slate-800 flex items-center justify-between">
                ...
                <a href="{{ route('gerbang.form') }}" class="text-gray-400 hover:text-red-500 text-xs bg-slate-700 p-2 rounded-lg transition duration-200"
                    title="Log Out"><i class="fa-solid fa-right-from-bracket"></i></a>
            </div>
        </aside>

        <div class="flex-1 flex flex-col h-full overflow-hidden">
            <header class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-8 shadow-xs shrink-0">
                <div class="flex items-center space-x-2.5">
                    <span class="text-slate-400 text-xs w-5 text-center">
                        <i class="fa-solid {{ $icon ?? 'fa-solid fa-folder-tree' }}"></i>
                    </span>
                    <span class="text-xs font-bold text-slate-800 tracking-wide uppercase">{{ $title ?? 'Beranda' }}</span>
                </div>
